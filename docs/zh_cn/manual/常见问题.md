...
完整 MAATree 软件压缩包命名格式为 "MAATree-`平台`-`架构`-`版本`.zip"，其余均为无法单独使用的“零部件”，请仔细阅读。  
~~在大部分情况下，您需要使用 x64 架构的 MAATree，即您需要下载 `MAATree-win-x86_64-vXXX.zip`，而非`MAATree-win-aarch64-vXXX.zip`。~~  
MAATree Windows 已不支持该架构
...
请到[项目Issues页面](https://github.com/caicai00001/MAATree/issues)反馈。
...
问题出现时提示**资源加载失败**。  
解决方法是删除整个文件夹，然后[重新下载](https://github.com/caicai00001/MAATree/releases)安装 MAATree 。
...
关闭 MAATree 后查找 `任务管理器` - `详细信息` 中有无名称包含 `adb` 的进程，如有，结束它后重试连接。
...
1. 到[项目Issues页面](https://github.com/caicai00001/MAATree/issues) ，**根据模板要求**提交相关材料。
