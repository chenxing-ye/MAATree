...
class BankPurchaseRecord(CustomAction):
    ...
    def run(
        ...
        with open("config/maatree_data.json") as f:
            data = json.load(f)

        data["bank"][item] = int(time.time() * 1000)

        with open("config/maatree_data.json", "w") as f:
...
class ModifyBankTaskList(CustomAction):
    ...
    def run(
        ...
        file_path = "config/maatree_data.json"
        default_data = {"bank": {}}

        if not os.path.exists(file_path):

            logger.warning("config/maatree_data.json 不存在，正在初始化")
